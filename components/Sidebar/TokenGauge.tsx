...
export const TokenGauge = forwardRef<TokenGaugeRef, {}>(function TokenGauge(props, ref) {
  ...
  useEffect(() => {
    if (remaining <= 1000 && remaining > 0) {
      toast.warning(`Token warning: Only ${remaining.toLocaleString()} tokens remaining today!`);
    } else if (remaining <= 0) {
      toast.error("Daily token limit reached! Token counter will reset tomorrow.");
    }
  }, [remaining]);
